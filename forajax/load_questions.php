...
$res=mysqli_query($conn,"SELECT * FROM questions WHERE category='$_SESSION[exam_category]' AND  question_no=$_GET[questionno]"); 
...
else
{
    ...
    <table>
        <tr>
           <td style="font-weight: bold; font-size: larger; padding-left: 5px" colspan="2">
           <?php echo "( ".$question_no ." ) ". $question; ?>
           </td>
        </tr>
    </table>

    <table style="margin-left: 20px; padding-top: 200px; font-size:larger;"  >
      <tr>
         <td>
            <input type="radio" name="rl" id="rl" value="<?php echo $optl; ?>" onclick="radioclick(this.value,<?php echo $question_no ?>)"
            <?php
               if ($ans==$opt1)
               {
                  echo "checked";
